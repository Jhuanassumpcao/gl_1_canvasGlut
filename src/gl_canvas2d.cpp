...
void CV::rectFill( Ponto p1, Ponto p2 )
{
   glBegin(GL_QUADS);
...
void CV::polygon(std::vector<Ponto> pontos)
{
   glBegin(GL_LINE_LOOP);
    for(auto& ponto : pontos){
        glVertex2d(ponto.x, ponto.y);
...
void CV::circle(Ponto p, float radius, int div )
{
   float ang = 0, x1, y1;
   float inc = PI_2/div;
   glBegin(GL_LINE_LOOP);
...
void CV::circleFill( Ponto p, float radius, int div )
{
   float ang = 0, x1, y1;
   float inc = PI_2/div;
   glBegin(GL_POLYGON);
